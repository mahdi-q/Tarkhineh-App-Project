...
function NavbarDropDown({ state, setState, text, items }) {
  ...
  return (
    <div ref={ref}>
      <button
        className="flex min-w-60 items-center justify-between gap-x-2 md:min-w-max"
      ...
      >
        {items.map((item) => (
          <li key={item.link} className={dropDownLinkClass}>
            <CustomNavLink
              to={item.link}
