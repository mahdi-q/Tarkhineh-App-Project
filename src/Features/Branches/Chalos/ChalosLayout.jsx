...
function ChalosLayout() {
  const [searchValue, setSearchValue] = useState("");
  const { chalosBranch } = useBranches();

  if (
    !chalosBranch ||
    !chalosBranch.special_foods ||
    chalosBranch.special_foods.length <= 0
  )
    return (
  ...
  return (
    <div className="minimum-height">
      ...
      <CardsSlider title="پیشنهاد ویژه" cards={chalosBranch.special_foods} />

      <CardsSlider
        title="غذاهای محبوب"
        cards={chalosBranch.famous_foods}
        isPrimary
      />

      <CardsSlider
        title="غذاهای غیر ایرانی"
        cards={chalosBranch.foreign_foods}
        isLast
      />

      <BranchInfo
        title={chalosBranch.title}
        image={chalosBranch.image}
        phone_numberes={chalosBranch.phone_numberes}
        location={chalosBranch.location}
        work_time={chalosBranch.work_time}
      />

      <CommentsSlider comments={chalosBranch.comments} />
    </div>
  );
}
export default ChalosLayout;
